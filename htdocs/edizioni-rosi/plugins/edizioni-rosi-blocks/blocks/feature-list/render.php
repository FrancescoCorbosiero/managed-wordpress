<?php
/**
 * Feature List Block — Server-side render
 * Numbered editorial values / services with decorative icons
 */
...
// Icon map — inline SVGs keyed by slug
$icons = array(
    ...
    'pen' => '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M17 3a2.85 2.83 0 1 1 4 4L7.5 20.5 2 22l1.5-5.5Z"/><path d="m15 5 4 4"/></svg>',
...
?>
<section class="er-features" data-er-reveal>
    <div class="er-features__header">
        <?php if (!empty($eyebrow)) : ?>
            <span class="er-features__eyebrow"><?php echo esc_html($eyebrow); ?></span>
        <?php endif; ?>

        <h2 class="er-features__title"><?php echo esc_html($title); ?></h2>

        <?php if (!empty($subtitle)) : ?>
            <p class="er-features__subtitle"><?php echo esc_html($subtitle); ?></p>
        <?php endif; ?>
    </div>

    <div class="er-features__grid">
        <?php foreach ($items as $index => $item) :
            if (empty($item['title'])) continue;
            $icon_key = $item['icon'] ?? 'book';
            $icon_svg = $icons[$icon_key] ?? $icons['book'];
        ?>
            <div class="er-features__item">
                <span class="er-features__number" aria-hidden="true"><?php echo esc_html(str_pad($index + 1, 2, '0', STR_PAD_LEFT)); ?></span>
                <div class="er-features__icon"><?php echo $icon_svg; ?></div>
                <h3 class="er-features__item-title"><?php echo esc_html($item['title']); ?></h3>
                <?php if (!empty($item['description'])) : ?>
                    <p class="er-features__item-text"><?php echo esc_html($item['description']); ?></p>
                <?php endif; ?>
            </div>
        <?php endforeach; ?>
    </div>
